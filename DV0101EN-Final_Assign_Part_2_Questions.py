...
app.layout = html.Div([
    ...
    html.Div([#TASK 2.2: Add two dropdown menus
        ...
        dcc.Dropdown(
            ...
            style={'width':'65%','padding':3,'font-size':20,'text-align-last':'center'}
        )
    ]),
...
#Plot 1 Automobile sales fluctuate over Recession Period (year wise)
        ...
        yearly_rec=recession_data.groupby(recession_data['Year'])['Automobile_Sales'].mean().reset_index()
        R_chart1 = dcc.Graph(
...
# Plot bar chart for average number of vehicles sold during the given year
        avr_vdata=yearly_data.groupby(data['Year'])['Automobile_Sales'].mean().reset_index()
        Y_chart3 = dcc.Graph(figure=px.histogram(yearly_data,
                            x='Year',
                            color= 'Vehicle_Type',
                            barmode='group',
...
# Total Advertisement Expenditure for each vehicle using pie chart
        exp_data=yearly_data.groupby(yearly_data['Vehicle_Type'])['Advertising_Expenditure'].sum().reset_index()
